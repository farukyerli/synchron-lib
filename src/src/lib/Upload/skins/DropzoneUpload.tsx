...
const DropZoneForm = (props: IProps) => {
    const { connection, classes, actions, refreshOnFilesChange = false, readOnly = false, maxFiles = 5 } = props;
    ...
    const onDelete = (index: number, data: any) => {
        ...
        dispatch({
            type: stateAction.onDelete, payload: {
                fileURLList: myArray.map((n, i) => ({ index: i, filename: n.filename })),
    ...
    return (
        ...
        >
            ...
            {state.fileURLList.length < maxFiles && !readOnly && <DropzoneCanvas
                {...props}
                onChange={(data) => {
                    dispatch({
